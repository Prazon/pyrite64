...
{
  class MathMax : public Base
  {
    public:
      constexpr static const char* NAME = ICON_MDI_ARROW_UP_BOLD " Max";
      ...
      MathMax()
      {
        ...
        addOUT<TypeValue>("Max", pinStyle(PinDataType::Float));
      }
      ...
      void build(BuildCtx &ctx) override
      {
        ...
        if (ctx.inValUUIDs && ctx.inValUUIDs->size() >= 2) {
          ...
          ctx.line(resVar + " = fmaxf((float)(" + a + "), (float)(" + b + "));");
        }
      }
  };
}
