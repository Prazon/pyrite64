...
{
  class MathMin : public Base
  {
    public:
      constexpr static const char* NAME = ICON_MDI_ARROW_DOWN_BOLD " Min";
      ...
      MathMin()
      {
        ...
        addOUT<TypeValue>("Min", pinStyle(PinDataType::Float));
      }
      ...
      void build(BuildCtx &ctx) override
      {
        ...
        if (ctx.inValUUIDs && ctx.inValUUIDs->size() >= 2) {
          ...
          ctx.line(resVar + " = fminf((float)(" + a + "), (float)(" + b + "));");
        }
      }
  };
}
