...
namespace Project::Graph::Node
{
  ...
  void ScriptNode::addInputPin(const PinDef &pin)
  {
    ...
    // Fork build pass: mark which input pins carry values (1) vs exec (0).
    valInputTypes.push_back(pin.value ? 1 : 0);
  ...
  {
    auto types = valueInputTypes();
    auto lits  = valueInputLiterals();
    auto *savedT = ctx.inValTypes;
    auto *savedF = ctx.inValFallbacks;
    ctx.inValTypes = &types;
    ctx.inValFallbacks = &lits;

    if(spec->prepareBuild) spec->prepareBuild(*this, ctx);
    if(spec->build) spec->build(*this, ctx);
    if(spec->value) {
      // Materialize the value output as the res_<uuid> globalVar that
      // table-based consumer nodes read; assigned at exec time so a value
      // node wired through exec follows the Compare-style pattern.
      std::string t = cTypeOf(firstValueOutType());
      std::string resVar = "res_" + Utils::toHex64(uuid);
      ctx.globalVar(t, resVar, t + "{}");
      ctx.line(resVar + " = " + spec->value(*this, ctx) + ";");
    }

    ctx.inValTypes = savedT;
    ctx.inValFallbacks = savedF;
  }

  void ScriptNode::buildAsPure(BuildCtx &ctx)
  {
    auto types = valueInputTypes();
    auto lits  = valueInputLiterals();
    auto *savedT = ctx.inValTypes;
    ...
    if(spec->prepareBuild) spec->prepareBuild(*this, ctx);
    if(spec->value) {
      // Pure pass: inline the expression straight into the initializer.
      std::string t = cTypeOf(firstValueOutType());
      ctx.globalVar(t, "res_" + Utils::toHex64(uuid), spec->value(*this, ctx));
    } else if(spec->build) {
      spec->build(*this, ctx);
    }
  ...
  std::string ScriptNode::value(BuildCtx &ctx)
  {
    ...
    ctx.inValTypes = &types;
    ctx.inValFallbacks = &lits;
    std::string out = spec->value ? spec->value(*this, ctx) : std::string{};
    ctx.inValTypes = savedT;
    ctx.inValFallbacks = savedF;
    return out;
  }
