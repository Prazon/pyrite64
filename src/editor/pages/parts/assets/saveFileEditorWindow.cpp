/**
* Per-asset SAVE_FILE editor window. Same shape as ResourceTypeEditorWindow.
*/
#include "saveFileEditorWindow.h"
...
bool Editor::SaveFileEditorWindow::draw(ImGuiID defDockId)
{
  auto &assetManager = ctx.project->getAssets();
  auto asset = assetManager.getEntryByUUID(assetUUID);
  if (!asset) return false;
  if (asset->type != Project::FileType::SAVE_FILE) return false;

  winName = "Save File: " + asset->name
    + "###SaveFileEditorWin_" + std::to_string(assetUUID);
  ...
  float rightW = std::clamp(fullAvail.x * splitFrac, minRightW,
                            std::max(minRightW, fullAvail.x - minLeftW - splitterW));
  float leftW  = std::max(minLeftW, fullAvail.x - splitterW - rightW);

  ImGui::BeginChild("##saveMain", ImVec2(leftW, 0), ImGuiChildFlags_Borders);
  if (asset->saveFileAsset) {
    Editor::SaveFileEditor::draw(*asset);
  } else {
    ImGui::TextDisabled("(save asset body missing)");
  }
  ImGui::EndChild();

  ImGui::SameLine();
  ImGui::InvisibleButton("##saveSplit", ImVec2(splitterW, -1));
  ...
  ImGui::SameLine();
  ImGui::BeginChild("##saveInspector", ImVec2(0, 0), ImGuiChildFlags_Borders);
...
void Editor::SaveFileEditorWindow::focus() const
{
